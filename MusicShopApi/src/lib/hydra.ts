...
export async function authMiddleware(req: Request, res: Response, next: NextFunction) {
  ...
  const basic = Buffer.from(`${clientId}:${clientSecret}`).toString('base64');
    ...
    if (!data.active) return res.status(401).json({ error: 'inactive token' });
    (req as any).subject = data.sub || 'client';
    const scopeStr: string = (data.scope || data.scp || '').toString();
    (req as any).scopes = scopeStr.split(' ').map((s: string) => s.trim()).filter(Boolean);
    next();
  } catch (e) {
    const anyErr: any = e;
...
export function requireScope(required: 'read' | 'write') {
