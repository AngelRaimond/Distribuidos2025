...
function hateoasInstrument(base: string, it: Instrument) {
  return {
    ...it,
    _links: {
...
router.get('/instruments', requireScope('read'), async (req: Request, res: Response) => {
  try {
    ...
    if (sort && ['nombre','marca','modelo','precio','anio','categoria'].includes(sort)) {
      list = [...list].sort((a: any, b: any) => (a as any)[sort] > (b as any)[sort] ? 1 : ((a as any)[sort] < (b as any)[sort] ? -1 : 0));
    }
...
router.get('/instruments/:id', requireScope('read'), async (req, res) => {
  ...
  try {
    ...
    const cached = await cacheGet<Instrument>(itemKey);
    if (cached) {
      const base = `${req.protocol}://${req.get('host')}`;
      res.set('X-Cache-Status', 'HIT');
    ...
    if (!it) return res.status(404).json({ error: 'not_found', message: `Instrument with id ${id} not found` });
    const base = `${req.protocol}://${req.get('host')}`;
...
router.put('/instruments/:id', requireScope('write'), async (req: Request, res: Response) => {
  const id = Number(req.params.id);
  if (!Number.isFinite(id) || id <= 0) return res.status(400).json({ error: 'bad_request', message: 'id must be a positive integer' });
  try {
    ...
    if (!updated) return res.status(404).json({ error: 'not_found', message: `Instrument with id ${id} not found` });
...
router.delete('/instruments/:id', requireScope('write'), async (req: Request, res: Response) => {
  const id = Number(req.params.id);
  if (!Number.isFinite(id) || id <= 0) return res.status(400).json({ error: 'bad_request', message: 'id must be a positive integer' });
  try {
    const ok = await soapDelete(id);
    if (!ok) return res.status(404).json({ error: 'not_found', message: `Instrument with id ${id} not found` });
    await cacheDel(`item:v1:${id}`);
    await cacheBump('list');
